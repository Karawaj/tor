...
#define MAYBE_SKIP 0
#else
#define MAYBE_SKIP TT_SKIP
#endif

#define THREAD_TEST(name)                                               \
  { #name, test_threads_##name, TT_FORK|MAYBE_SKIP, NULL, NULL }

struct testcase_t thread_tests[] = {
  THREAD_TEST(basic),
  { "conditionvar", test_threads_conditionvar, TT_FORK|MAYBE_SKIP,
    &passthrough_setup, (void*)"no-tv" },
  { "conditionvar_timeout", test_threads_conditionvar, TT_FORK|MAYBE_SKIP,
